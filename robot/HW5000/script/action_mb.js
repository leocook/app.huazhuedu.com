$(document).ready(function () {
    ...
    $("#blue_35").on("touchend ", function () {
        ...
        if(man.stat.set_start_mod===1){
            ...
            if(man.scrn.start_mod.auto===0){
                man.turn_lamp('lamp_33',0);
            }else{
                man.turn_lamp('lamp_33',1);
            }
            man.draw_clean_scrn();
            man.draw_header();
            man.draw_footer();
    ...
    $(".q_start").on("click", function () {
        man.get_questions(man);
        checker = setInterval(man.check_answer.bind(man),2000);
    });

    $(".q_quit").on("click", function () {
        window.location.href="/index.php";
    });

    $(".q_pre").on("click", function () {

    });

    $(".q_next").on("click", function () {
       counter+=1;
       var q = JSON.parse(questions[counter]['param']);
       man.to_question_stat(q);
       $('.q_body').text(questions[counter]['question']);
        checker = setInterval(man.check_answer.bind(man),2000);
    });
...
Robot.prototype.to_question_stat = function(q){
    for(var lamp_i in q['lamp']){
        this.lamp[lamp_i] = q['lamp'][lamp_i];
    }
    for(var mul_i  in q['mul']){
        this.mul[mul_i] = q['mul'][mul_i];
    }
    for(var stat_i  in q['stat']){
        this.stat[stat_i] = q['stat'][stat_i];
    }
    if(q['scrn'] !==''){
...
Robot.prototype.get_questions = function(man){
    $.ajax({
        url: "get.php",
        type: "get",
        data: '',
        ...
        success: function (data) {
            $("#q_start_pan").addClass('hidden');
            $("#q_pan").removeClass('hidden');
            $(".q_title").text("..:: 操 作 题 ::..");
            questions=JSON.parse(data);
            var q = JSON.parse(questions[0]['param']);
            man.to_question_stat(q);
            $('.q_body').text(questions[0]['question']);
        }
    });
};

Robot.prototype.check_answer = function(man){
    var answer = JSON.parse(questions[counter]['answer']);
    for(var stat_i  in answer['stat']){
        //alert(this.stat[stat_i]);
        //alert(answer['stat'][stat_i]);
        if(parseInt(this.stat[stat_i],10) !== parseInt(answer['stat'][stat_i],10)){
            return;
        }
    }
    for(var lamp_i  in answer['lamp']){
        if(parseInt(this.lamp[lamp_i],10) !== parseInt(answer['lamp'][lamp_i],10)){
            return;
        }
    }
    alert("恭喜您答对了，请点击下一题");
...
Robot.prototype.reset=function () {
    ...
    this.timer.normal[12]=setTimeout(this.to_default.bind(this),6000);
