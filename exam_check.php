...
$mysqli = get_conn();
$old_wr = get_user_wrong_set($mysqli,$_SESSION["mobile"]);
if($old_wr){
    $new_wr = dist_str($wr_insert,$old_wr);
    update_wrong_set($mysqli,$new_wr,$_SESSION["mobile"]);
}else{
    insert_wrong_set($mysqli,$wr_insert,$_SESSION["mobile"]);
}
insert_score($mysqli,$_SESSION["mobile"],$r_score);
record($mysqli,2,2,$r_score.'_'.$totle_score);
close_conn($mysqli);
$_SESSION['exam']='0';
